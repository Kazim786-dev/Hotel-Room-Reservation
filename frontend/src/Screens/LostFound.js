...
const LostFound = () => {
    ...
    const fetchLostItems = async () => {
        try {
            const response = await fetch('http://localhost:3001/lostAndfound/get-lost-items');
            if (response.ok) {
    ...
    const fetchSearchResults = async (query) => {
        try {
            const response = await fetch(`http://localhost:3001/lostAndfound/search-item?name=${query}`);
            const data = await response.json();
    ...
    const handleAddLostItem = async (name, description, location) => {
        try {
          const response = await fetch('http://localhost:3001/lostAndfound/add-item', {
            method: 'POST',
            headers: {
