...
const InventoryDashboard = () => {
  ...
  const fetchInventoryItems = async () => {
    try {
      const response = await fetch('http://localhost:3001/inventory/get-inventory');
  ...
  const handleDeleteItem = (itemId) => {
    fetch(`http://localhost:3001/inventory/remove-item?id=${itemId}`, {
      method: 'GET',
    })
      .then((response) => {
        if (response.ok) {
    ...
    const handleEditItem = (itemId) => {
        ...
        if (newQty !== null) {
          fetch('http://localhost:3001/inventory/update-item', {
            method: 'POST',
            headers: {
              'Content-Type': 'application/json',
            },
      ...
      const handleOrderItem = (itemId) => {
        ...
        if (qty !== null) {
          fetch('http://localhost:3001/inventory/place-order', {
            method: 'POST',
            headers: {
              'Content-Type': 'application/json',
            },
      ...
      const handleFetchOrders = async () => {
        try {
          const response = await fetch('http://localhost:3001/inventory/get-orders');
          const data = await response.json();
          setOrders(data);
        } catch (error) {
